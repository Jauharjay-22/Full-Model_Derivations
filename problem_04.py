"""
Problem 4: Non-homogeneous First-Order Linear ODE — Option A (3×2 Grid)
  dy/dx = x + y,  y(0) = 0,  x in [0, 1]
  Exact solution: y(x) = exp(x) - x - 1
...
for h in step_sizes:
    N = int(round(1.0 / h))
    x_rk4, y_rk4 = rk4(f, 0.0, 0.0, h, N)
    x_apc, y_apc = adams_pc(f, 0.0, 0.0, h, N)
    results[h] = {
...
fig.suptitle(r"Problem 4: Non-homogeneous ODE  —  $dy/dx = x + y$,  $y(0)=0$",
...
plt.tight_layout()
plt.savefig("problem4_option_a_grid.png", dpi=150, bbox_inches='tight')
print("Saved -> problem4_option_a_grid.png")
plt.show()
