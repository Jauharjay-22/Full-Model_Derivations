"""
Problem 3: Polynomial and Exponential Components — Option A (3×2 Grid)
  dy/dx = y - x^2 + 1,  y(0) = 0.5,  x in [0, 1]
  Exact solution: y(x) = (x+1)^2 - 0.5*exp(x)
...
for h in step_sizes:
    N = int(round(1.0 / h))
    x_rk4, y_rk4 = rk4(f, 0.0, 0.5, h, N)
    x_apc, y_apc = adams_pc(f, 0.0, 0.5, h, N)
    results[h] = {
...
fig.suptitle(r"Problem 3: Poly.+Exp.  —  $dy/dx = y - x^2 + 1$,  $y(0)=0.5$",
...
for row, h in enumerate(step_sizes):
    ...
    # Left: solution curve
    ax_sol = axes[row, 0]
    ax_sol.plot(x_fine, exact(x_fine), 'k-', lw=2,
                label=r'Exact $(x+1)^2 - 0.5e^x$')
...
plt.tight_layout()
plt.savefig("problem3_option_a_grid.png", dpi=150, bbox_inches='tight')
print("Saved -> problem3_option_a_grid.png")
plt.show()
