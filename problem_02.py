"""
Problem 2: Gaussian Decay — Option A (3×2 Grid)
  dy/dx = -2xy,  y(0) = 1,  x in [0, 1]
  Exact solution: y(x) = exp(-x^2)
...
fig.suptitle("Problem 2: Gaussian Decay  —  $dy/dx = -2xy$,  $y(0)=1$",
...
for row, h in enumerate(step_sizes):
    ...
    # Left: solution curve
    ax_sol = axes[row, 0]
    ax_sol.plot(x_fine, exact(x_fine), 'k-', lw=2, label='Exact $e^{-x^2}$')
...
plt.tight_layout()
plt.savefig("Problem_02.png", dpi=150, bbox_inches='tight')
print("Saved -> option_a_grid.png.png")
plt.show()
